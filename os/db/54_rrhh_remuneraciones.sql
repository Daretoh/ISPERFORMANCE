...
-- + Remuneraciones mensuales (extras/descuentos, bono por autos y ajuste por día).
...
-- 3) Remuneraciones: extras / descuentos / anticipos por trabajador y mes
create table if not exists remuneracion_items (
  id            uuid primary key default gen_random_uuid(),
  trabajador_id uuid not null references trabajadores(id) on delete cascade,
  periodo       date not null,                     -- 1er día del mes (se paga mensual)
  tipo          text not null,                     -- bono | descuento | otro | sin_bono_auto
  concepto      text,
  monto         numeric not null default 0,
  autor         text,
  created_at    timestamptz default now()
);
create index if not exists remu_items_periodo_idx on remuneracion_items (periodo);
